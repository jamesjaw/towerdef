...
PlateletTurret::PlateletTurret(float x, float y) :
	Turret("play/turret-2.png", x, y,20, Price, 0.5,0,3) {
...
void PlateletTurret::CreateBullet() {
	...
	getPlayScene()->EffectGroup->AddNewObject(new shootingEffect(Position.x+50, Position.y +20+ normalized.y * 36 - normal.y * 6)); //= =
...
void PlateletTurret::Update(float deltaTime){
	...
	//
	for (auto& it : scene->EnemyGroup->GetObjects()) {
            if (it->Position.x >= Position.x  && it->Position.x < Position.x+scene->BlockSize -50 && it->Position.y >= Position.y  && it->Position.y < Position.y+scene->BlockSize) {
                Target = dynamic_cast<Enemy*>(it);
                Target->speed_eff(0);
			}
    }
